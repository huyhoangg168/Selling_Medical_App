package com.example.clientsellingmedicine.activity.coupon;
...
public class ExchangeFragment extends Fragment implements IOnButtonExchangeCouponClickListener {
        ...
        request.enqueue(new Callback<UserDTO>() {
            ...
            public void onResponse(Call<UserDTO> call, Response<UserDTO> response) {
                if(response.isSuccessful()){
                    ...
                    navigateToLogin(); //Nếu chưa đăng nhập chuyển đến màn login
                } else {
            ...
            public void onResponse(Call<List<CouponDTO>> call, Response<List<CouponDTO>> response) {
                if(response.isSuccessful()){
                    if(response.body().size()>0){
                        couponAdapter = new couponAdapter(response.body(), ExchangeFragment.this); //Hiển thị danh sách mã gg lên couponAdapter
    ...
    //Hàm xử lý khi người dùng ấn đổi mã gg
    @Override
